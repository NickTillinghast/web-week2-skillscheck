...
let doubled = myNumbers.map(function(value, index, arr) {
  return value + value;
});
console.log(doubled);
...
const total = myNumbers.reduce(function(total, current) {
...
let myNumbersIndex = [];
myNumbers.forEach(function(array, index) {
...
let forTheLoveOfGeorge = notGeorge.map(function(value, index, arr) {
  return (value = "George");
});
...
let totallyAwesome = [];
people.reduce(function(total, current) {
  people[i].awesomeLevel = current + total;
  return total + current;
});
